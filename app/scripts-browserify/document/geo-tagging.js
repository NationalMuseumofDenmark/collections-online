...
function hideMap() {
  ...
  $editCoordinates.removeAttr('disabled');
}
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.status === 'success') {
        ...
        Snackbar.info('Gemt! Men endnu ikke opdateret her');
        contributionCounter.contributionAdded();
      } else {
        Snackbar.error('Der skete en fejl - prøv igen');
      }
    },
    error: function(response) {
      var err = response.responseJSON;
      Snackbar.error(err.message || 'Der skete en fejl - prøv igen');
      $('.map-buttons .hide-map').show();
      $('.map-buttons .save-coordinates')
        .removeAttr('disabled')
        .removeClass('loading');
    }
  });
