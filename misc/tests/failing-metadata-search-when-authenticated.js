...
function performTest(url, ids) {
	ids.forEach(function(id) {
		var named_parameters = { 'querystring': 'ID is \"'+id+'\"' };
		return request.post({
			url: url,
			...
			assets.forEach(function(asset) {
				if(asset.id !== id) {
					console.log('Test failed! Got ' +asset.id+ ' when requesting '+id);
				}
			});
		});
	});
...
console.log('Requesting without a jsessionid: The test is passing.');
performTest(METADATA_SEARCH_VIEW_URL, IDs);

request.post({ url: SESSION_OPEN_URL },
	function(is_error, response, body) {

	console.log('Requesting with a non-authenticated jsessionid: The test is passing.');
	var jsessionid = JSON.parse(body).jsessionid;
	performTest(METADATA_SEARCH_VIEW_URL+';jsessionid='+jsessionid, IDs);

});

request.post({ url: SESSION_OPEN_URL, form: { user: USER, password: PASSWORD } },
	function(is_error, response, body) {

	console.log('Requesting with an authenticated jsessionid: The test is failing.');
	var jsessionid = JSON.parse(body).jsessionid;
	performTest(METADATA_SEARCH_VIEW_URL+';jsessionid='+jsessionid, IDs);
