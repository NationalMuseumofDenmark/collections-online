...
function extendAndIndexAsset(state, subAssetMetadata, masterAssetMetadata) {
	...
	var extendedMetadata = assetMapping.extend_metadata(subAssetMetadata, masterAssetMetadata);
 ...
 */
function inheritMetadata(state) {
	console.log('Inheriting metadata from master assets of',
							state.indexedAssetIds.length,
							'assets');

	var assetIdQueue = state.indexedAssetIds.filter(function(id) {
	...
	function updateNextAssetFromRelations() {
		...
		if(assetId in handledAssetIds && handledAssetIds[assetId] > MAX_RECURRANCES) {
			...
			state.es.get({
				...
				var newlyIndexAssetIds = updateMetadataFromRelations(state, assetMetadata);
...
module.exports = inheritMetadata;