...
const SORT_OPTIONS = {
  ...
  'UPD_D': {
    ...
    method: {'modification_time': 'desc'}
  },
  'UPD_A': {
    label: 'Opdateret: Ældste',
    method: {'modification_time': 'asc'}
  },
  'ACCT_D': {
    label: 'Optagelsestid: Nyeste',
    method: {'acceptance_time_from': 'desc'}
  },
  'ACCT_A': {
    label: 'Optagelsestid: Ældste',
    method: {'acceptance_time_from': 'asc'}
  },
  'CREA_D': {
    label: 'Fremstillingstid: Nyeste',
    method: {'creation_time_from': 'desc'}
  },
  'CREA_A': {
    label: 'Fremstillingstid: Ældste',
    method: {'creation_time_from': 'asc'}
  }
...
function prepareFilters(req) {
  ...
  var sorting = req.query.sort || 'REL';
  var params = {};
...
function search(params, freetext, offset) {
  ...
  var sortMethod = SORT_OPTIONS[params.sorting];
  if (sortMethod) {
...
function pageUrls(filters, category) {
  ...
  var noSortingUrl = '?' + querystring.stringify(args);
  ...
  .then(function(search_result) {
    ...
    for (var i=0; i < results.length; ++i) {
      real_results.push({
        ...
        type: asset_player.determinePlayer( results[i] )
      });
    }
    ...
    res.render('search', {
      ...
      noSortingUrl: urls.noSortingUrl
    });
    res.end();
  });
};
