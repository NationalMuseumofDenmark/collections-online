...
function search(es_client, params, freetext, offset) {
  ...
  if('geotagged' in params) {
    ...
    if(params.geotagged) {
      ...
      query.body.filter.and.push({
        missing: {field: 'latitude'}
      });
      query.body.filter.and.push({
        missing: {field: 'longitude'}
      });
    }
  }
...
exports.index = function index(req, res) {
    ...
    .spread(function(results, total_results) {
      ...
      res.render('search', {
          req: req,
          infinitepath: 'null/infinite/?p=2&q=' + querystring.escape(filters.freetext),
          results: results,
          filters: filters
      });
      res.end();
...
exports.catalog = function catalog(req, res, next) {
  ...
  .spread(function(results, total_results) {
      ...
      if(metadata.length === 0) {
          var err = new Error('Found no assets.');
          err.status = 404;
          next(err);
      } else {
        metadata = metadata[0];
