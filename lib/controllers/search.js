...
exports.index = function index(req, res) {
    ...
    search(es_client, {}, freetext, 0,
      ...
      if (freetext != undefined) {
        var titleheading = 'Søgning på "' + freetext + '" (' + final_results.total_results + ' emner)';
      } else {
        var titleheading = 'Søg og udforsk Nationalmuseets Samlinger i alt ' + final_results.total_results + ' emner';
      }
...
exports.catalog = function catalog(req, res) {
  ...
  search(es_client, query_params, '', offset,
    function(results, offset, total_results) {
      ...
      if (req.params.catalog != undefined && category != undefined) {
        var titleheading = 'Din søgning resulterede i følgende resultater';
      } else {
        var titleheading = metadata.name + ' (' + final_results.total_results + ' emner)';
      }

      res.render('search', {
        req: req,
        titleheading: titleheading,
        infinitepath: 'infinite/' + req.param('catalog') + '/2?cat=' + category,
        results: final_results.results
      });
    }
  );
