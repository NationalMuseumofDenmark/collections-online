...
exports.result = function index(req, res) {
  ...
  .spread(function(results, total_results) {
    var final_results = {
...
exports.catalog = function catalog(req, res, next) {
  ...
  .spread(function(results, total_results) {
    ...
    var metadata = catalogs.filter(function(c) {
      return c.alias === req.params.catalog;
    }).map(function(c) {
      return {
        name: c.name,
        alias: c.alias,
    ...
    if(metadata.length === 0) {
      throw new Error('The provided catalog alias does not match any catalogs.');
    } else {
      metadata = metadata[0];

      var category = filters.params.category;
      if (req.params.catalog && category) {
        // Retrieve the breadcrumbs
        var path = categories[req.params.catalog].get_path(category);
      ...
      res.render('search', {
        req: req,
        infinitepath: req.param('catalog') + '/infinite' + qs,
