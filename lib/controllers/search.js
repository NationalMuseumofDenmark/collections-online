...
/**
 * Get a search result as data instead of rendered markup
 */
exports.json = function(req, res, next) {
  if(!req.body || !req.is('json')) {
    throw new Error('Expected a JSON request with a body');
  }
  var params = req.body.params || {};
  var freetext = req.body.freetext || '';
  var offset = req.body.offset || 0;
  var sorting = req.body.sorting || 'relevance';

  return search(params, freetext, offset, sorting)
  .then(function(response) {
    return transformResponse(req, response);
  }).then(function(result) {
    res.json(result);
