...
exports.download = function(req, res, next) {
  ...
  // Try to parse the size to an integer - if that works, override the size
  let nummericSize = size ? parseInt(size, 10) : null;
  if (nummericSize) {
    size = nummericSize;
  }
  ...
  if(size && POSSIBLE_SIZES && POSSIBLE_SIZES.indexOf(size) === -1) {
    throw new Error('The size is must be one of ' + POSSIBLE_SIZES + ' given: "'
                    + size + '"');
  }
  ...
  res.writeHead = function(statusCode, reasonPhrase, headers) {
    if(statusCode === 200 && proxyRequest.response) {
      ...
      if(contentDisposition) {
        ...
        if(size) {
          let sizePostfix = size;
          if(typeof(sizePostfix) === 'string') {
            // Remove JPEG from e.g. kbh-museum-26893-originalJPEG.jpg
            sizePostfix = sizePostfix.replace('JPEG', '');
          }
          filename += '-' + sizePostfix;
        }
