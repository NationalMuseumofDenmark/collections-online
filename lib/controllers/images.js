  ...
  res.writeHead = function(statusCode, reasonPhrase, headers) {
    if(statusCode === 200 && proxyRequest.response) {
      ...
      if(contentDisposition) {
        extension = contentDisposition.match(contentDispositionRegexp);
      }
      if(extension) {
...
exports.thumbnail = function(req, res, next) {
  ...
  let cacheKey = JSON.stringify(req.params);
  waterStream.cached(cacheKey, res, function(cacheRes) {
    console.log('Cache miss on key:', cacheKey);
    ...
    .then(function(metadata) {
      ...
      proxyReq.on('error', next).on('response', function(response) {
        if(response.statusCode === 200) {
          var t = waterStream.transformation(watermark, size, positionFunction);
          proxyReq.pipe(t).pipe(cacheRes);
        } else {
          // res.type('png');
          getErrorPlaceholderStream().pipe(cacheRes);
        }
      });
    }).then(null, next);
  });
