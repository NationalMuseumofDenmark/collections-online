...
var cip = require('../services/natmus-cip'),
    ...
    assetLayout = require('../config/asset-layout.json'),
    jade = require('jade');

var simpleTemplate = jade.compileFile('./app/views/asset-rows/simple.jade');

const ASSET_ROW_TYPES = {
  'simple': (row, metadata) => {
    var value;
    if (row.fields in metadata) {
      value = metadata[row.fields];
    }
    if (value) {
  ...
  'date-interval': (row, metadata) => {
    return 'Not implemented';
  },
  'changeable-coordinates': (row, metadata) => {
    return 'Not implemented';
  },
  'license': (row, metadata) => {
    return 'Not implemented';
  },
  'file-size-mb': (row, metadata) => {
    return 'Not implemented';
  },
  'image-dimensions-px': (row, metadata) => {
    return 'Not implemented';
  },
  'image-dimensions-cm': (row, metadata) => {
    return 'Not implemented';
  }
};
...
assetLayout.sections.forEach((section) => {
  section.rows.forEach((row) => {
    ...
    if (Object.keys(ASSET_ROW_TYPES).indexOf(row.type) === -1) {
      throw new Error('Unexpected item type: ' + row.type);
    } else {
      row.render = (metadata) => {
        return ASSET_ROW_TYPES[row.type](row, metadata);
      };
    }
  });
});

const OUTDOORS_TAG = 'udendørs';
...
exports.index = function(req, res, next) {
  ...
  .then(function(metadata) {
    ...
    if (config.enableGeotagging) {
      ...
      var visionTags = metadata.tags_vision || [];
      var outdoors = visionTags.indexOf(OUTDOORS_TAG) !== -1;
      showGeotagging = (rightCatalog || outdoors) &&
                       !metadata.google_maps_coordinates;
