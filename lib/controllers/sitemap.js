...
exports.index = function(req, res, next) {
  const hostBase = req.headers['x-forwarded-host'] || req.get('host');
  const baseUrl = 'http://' + hostBase + '/';
  ...
  ds.search({
    ...
    collections.forEach(collection => {
      collection.ids.buckets.forEach(idBucket => {
        if(idBucket.doc_count > 0) {
          const fromId = idBucket.key;
          const qs = querystring.stringify({
            fromId,
            toId: fromId + SITEMAP_ASSET_LIMIT
          });
          const path = collection.key + '/sitemap.xml?' + qs;
          sitemaps.push({
