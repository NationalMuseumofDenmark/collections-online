...
exports.index = function(req, res, next) {
  return documentController.get(req, 'asset').then((metadata) => {
    ...
    if (relatedAssets.length > 0) {
      ...
      relatedAssets.forEach(function(asset) {
        // TODO: Make sure ids on related assets are prefixed
        let assetId;
        if(asset.id.indexOf('-') !== -1) {
          assetId = asset.id;
        } else {
          assetId = metadata.collection + '-' + asset.id;
        }
        relatedAssetById[assetId] = asset;
      });
  ...
  .then(function(metadata) {
    ...
    // Determine if zoomable
    var zoomable = metadata.width_px > 768 ? true : false;
    ...
    return {
      ...
      'showGeotagging': showGeotagging,
      'showCallToAction': showCallToAction,
      'sources': assetPlayer.generateSources(req,
                                             player,
                                             req.baseUrl,
                                             metadata),
      'req': req,
