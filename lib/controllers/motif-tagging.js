...
let Q = require('q');
let cip = require('../services/natmus-cip');
let es = require('../services/elasticsearch');
let indexAsset = require('../../indexing/processing/asset');
...
function fetchSuggestions(imageURL) {
  ...
  return Q.all(suggestionsPromises).then(function(suggestedTags) {
    ...
    var tags = _.union.apply(null, suggestedTags).filter(function(tag) {
      return tag == false; // Filter out undefined, null and ''
    });
...
function updateIndex(metadata) {
  ...
  return indexAsset(indexingState, metadata, []);
}
...
exports.saveCrowdTag = function(req, res, next) {
  ...
  .then(function(metadata) {
    ...
    return saveToCip(catalog, id, values).then(function() {
      return metadata;
    });
  })
  .then(function(metadata) {
