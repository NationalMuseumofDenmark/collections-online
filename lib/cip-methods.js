...
exports.init_session = function init_session(force_init) {
    ...
    if(nm && !force_init) {
	    ...
	    nm.session_open(
            process.env.CIP_USERNAME, process.env.CIP_PASSWORD,
            function( ) {
...
exports.get_asset = function get_asset(nm, catalog, id) {
    ...
    if(!asset) {
        ...
        nm.advancedsearch({catalog: {alias: catalog}}, id_string, undefined, null, function(result) {
            if(result) {
                cache.put(cache_key, result, CACHE_TIME);
                // console.log('Found ' + result.fields.id + ' when requesting '+id_string);
                deferred.resolve(result);
            } else {
                deferred.reject( new Error("The CIP didn't return exactly one asset.") );
            }
        }, function(err) {
