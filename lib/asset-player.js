...
function determinePlayer(asset) {
  ...
  if (format === 'gif image') {
    ...
    var isPrimaryRotationImage = asset.in_artifact_rotation_series &&
                                 asset.artifact_rotation_series_rank === 0;
    if (isPrimaryRotationImage) {
...
function generateRotationSources(req, metadata) {
  ...
  for (var r in rotaryAssets) {
    ...
    if (id) {
      var imageURL = '/' + metadata.catalog + '/asset/' + id + '/image/';
...
function generateSources(req, player, url, metadata) {
  ...
  if (player === 'image-single') {
    var itemTitle = metadata.title || 'Ingen titel';
