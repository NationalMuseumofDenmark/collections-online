...
function determine_player(asset) {
    if(!asset.filename) {
      throw new Error('Cannot determine player from an asset without a filename.');
    }
    var filename = asset.filename.toLowerCase();
    if( filename.match('.tif$') || filename.match('.tiff$') ||
        filename.match('.jpg$') || filename.match('.jpeg$') ||
        filename.match('.pdf$') || // TODO: Investigate if PDFs are actually shown.
        filename.match('.cr2$') /* Canon Raw */ ) {

        var in_rotation_category = asset.categories.some(function(category) {
            return category.name === config.cip_rotation_category_name;
        });

        if(in_rotation_category) {
        ...
        throw new Error('This type of asset has no player associated.');
    }
}
