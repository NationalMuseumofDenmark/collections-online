 ...
 *
 * @param {Object} metadata The object to look for values in.
 * @param {string} path The path used when looking for values.
 * @param {string} format Optional Moment date format to use when parsing string
 *                 dates.
 */
helpers.getDate = function(metadata, path, format = undefined) {
  // Get the raw value.
  let property = _.get(metadata, path);
  ...
  if(typeof(property) === 'string') {
    ...
    // Attempt parse.
    let date = moment(property, format);
    if (!date.isValid()) {
...
const appDirHelpersPath = config.customizationPath + '/shared/helpers.js';
