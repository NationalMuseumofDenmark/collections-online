...
const TEMPLATE_PATH = __dirname + '/../app/views/asset-row-types/';
var templates = {};
fs.readdirSync(TEMPLATE_PATH).forEach((templateFile) => {
  var templateName = templateFile.substring(0, templateFile.indexOf('.'));
  templates[templateName] = jade.compileFile(TEMPLATE_PATH + templateFile);
});
...
const TYPES = {
  'simple': {
    'hasValue': (row, options, metadata) => {
      var withMetadata = templates.simple({
        row: row,
        options: options,
        metadata: metadata,
        helpers: helpers
      });
      var withoutMetadata = templates.simple({
        row: row,
        options: options,
        metadata: {},
        helpers: helpers
