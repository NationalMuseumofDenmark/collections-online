...
module.exports = function(config) {
  ...
  function resolvePath(relativePathToResolve, source) {
    ...
    }).filter(candidatePath => candidatePath !== source); // Prevents recursion
  ...
  return {
    resolve: function(filename, source, options) {
      ...
      return resolvePath(path.join(relativeFilepath, filename), source);
    }
  };
};
