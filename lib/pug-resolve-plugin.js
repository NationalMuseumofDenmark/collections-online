...
module.exports = function(config) {
  ...
  function resolvePath(relativePathToResolve) {
    ...
    // Append candidate paths where .pug is replaced with .jade
    var candidateJadePaths = candidatePaths.filter((path) => {
      // The path must contain .pug
      return path.indexOf('.pug') !== -1;
    }).map((path) => {
      return path.replace('.pug', '.jade');
    });

    candidatePaths = candidatePaths.concat(candidateJadePaths);
    ...
    if (!resolvedPath) {
      throw new Error('Failed to resolve ' + relativePathToResolve);
    } else {
      return resolvedPath;
    }
  }
