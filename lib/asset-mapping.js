...
var result_mapping = [
    ...
    {'key': '{9aef60d3-38b4-4e47-8104-d018b83ea6c6}',
     'short': 'object_number' // Genstandsnummer
     //'short': 'unitno'
    },
...
function extend_from_master(nm, metadata) {
    ...
    // TODO: Remove this console.log when debugged.
    console.log('Extending metadata of asset '+metadata.id+' with master asset ' + master_asset_id);

    return cip.get_asset(nm, metadata.catalog, master_asset_id)
    .then(function(master_asset) {
        console.log('Before the format_result of ' + master_asset_id + ' we had ' + master_asset.fields.id);
        var master_asset_metadata = format_result(master_asset.fields);
        // TODO: Remove this console.log when debugged.
        console.log('Fetch master asset "'+ master_asset_metadata.short_title+'" (' + master_asset_metadata.id +') to be extending ' +metadata.id);
