...
function extend_metadata(nm, catalog_alias, asset, formatted_item) {
    return Q.Promise(function(resolve, reject) {
        // Get the asset's relations, to check for a master.
        // Cosider if this promise chain can be cleaned up.
        cip.get_related_assets(asset, "iscontainedin").then(function(iscontainedin_relations) {
            cip.get_related_assets(asset, "isvariantof").then(function(isvariantof_relations) {
                cip.get_related_assets(asset, "isalternateof").then(function(isalternateof_relations) {
                    ...
                    if(relation_ids.length == 1) {
                        ...
                        cip.get_asset(nm, catalog_alias, master_asset_id)
                            ...
                            if(assets.length === 1) {
                                ...
                                for(var f in formatted_item) {
                                    ...
                                    if(overwriting_value !== null && overwriting_value !== "") {
                                        console.log("Overwriting", f, "with", overwriting_value);
                                ...
                                reject( new Error("The asset referenced less or more than a single asset.") );
                            }
                        });
                    } else {
                        resolve( fields );
        ...
        // Get the master's metadata
        // Overwrite the temporary copy of the masters metadata with the content of
        // fields, and return the new extended metadata.
    });
}
