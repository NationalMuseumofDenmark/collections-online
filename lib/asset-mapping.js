...
function extend_metadata(subAssetMetadata, masterAssetMetadata) {
...
function extend_from_master(nm, metadata) {
  ...
  // master asset ids.
  if (!metadata.related_master_assets || metadata.related_master_assets.length ===
    0) {
    return metadata;
  }
  ...
  return cip.getAsset(nm, metadata.catalog, master_asset_id)
    .then(function(master_asset) {
      var master_asset_metadata = format_result(master_asset.fields);
      return extend_metadata(metadata, master_asset_metadata);
    });
}
